...
def test_load_users(mocked_input, empty_collections):
    ...
    assert len(menu.user_collection['users']) == 3

@mock.patch('builtins.input', wraps=MultiInputMock(
    [str(HERE / "data_for_tests.py"),  # not a csv file
     ]))
def test_load_users_invalid(mocked_input, empty_collections):
    # just checking that it ran without error
    result = menu.load_users()
    assert result is None

    assert len(menu.user_collection['users']) == 0


@mock.patch('builtins.input', wraps=MultiInputMock(
    [str(HERE / "non-existant.csv"),  # not a csv file
     ]))
def test_load_users_non_existant_file(mocked_input, empty_collections):
    # just checking that it ran without error
    result = menu.load_users()
    assert result is None
...
def test_load_status_updates(mocked_input, empty_collections):
    ...
    assert result is None
    assert len(menu.status_collection['statuses']) == 3
...
def test_add_status(mocked_input, full_db):
    ...
    status = main.search_status('1234567',
                                menu.status_collection)
    assert status['status_id'] == '1234567'
    assert status['user_id'] == 'fjones34'
    assert status['status_text'] == "An arbitrary status message"
...
def test_add_status_already_there(mocked_input, full_collections):
    ...
    # But did it get changed?
    status = main.search_status('st55555',
                                menu.status_collection)
    assert status['status_id'] == 'st55555'
    assert status['status_text'] != 'An entirely new message'
...
def test_update_status(mocked_input, full_collections):
    ...
    status = main.search_status('st55555',
                                menu.status_collection)
    assert status['status_id'] == 'st55555'
    assert status['status_text'] == "new updated message"
