...
# so they will be in the global namespace
...
def load_users():
    ...
    try:
        ...
        print(f"File: {filename} not there -- try again")
...
def add_status():
    ...
    if not main.add_status(user_id=user_id,
                           status_id=status_id,
                           status_text=status_text,
                           dataset=status_collection):
...
# pylint: disable=global-variable-undefined, invalid-name, global-statement, redefined-outer-name
def init_collections(dataset):
    '''
    initialize the user and status collections
    '''
    global user_collection
    global status_collection
    user_collection = status_collection = dataset
...
if __name__ == '__main__':  # pragma: no cover
    ...
    if clear:
        print("Starting up with empty database")
    else:
        print("Starting with existing db: 'social.db'")
    dataset = socialnetwork_model.get_dataset('social.db', clear)
    init_collections(dataset)
    mainloop()
