...
@mock.patch('builtins.input', wraps=MultiInputMock(
    [str(HERE / "non-existant-file.csv"),
     ]))
def test_load_users_bad_filename(mocked_input, empty_collections):
    # just checking that it ran without error
    result = menu.load_users()
    assert result is None

    assert len(menu.user_collection) == 0
...
def test_load_status_updates(mocked_input, empty_collections):
    ...
    assert main.search_status('dave03_00001',
                            menu.status_collection).status_id == 'dave03_00001'
...
def test_add_status(mocked_input, full_db):
    ...
    status = menu.status_collection.search_status(EXAMPLE_STATUS.status_id)
    assert status.status_id == EXAMPLE_STATUS.status_id
    assert status.user_id.user_id == EXAMPLE_STATUS.user_id
...
def test_update_status(mocked_input, full_collections):
    ...
    # did it update properly?
    status = menu.status_collection.search_status('st12355')

    assert status.status_text == "darn -- coverage 90% more tests to run"
